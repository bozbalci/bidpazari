...
from bidpazari.core.exceptions import (BiddingErrorReason, BiddingNotAllowed,
                                       ...
                                       InvalidPassword, UserVerificationError)
from bidpazari.core.models import Item, Transaction, User, UserHasItem
from bidpazari.core.runtime import RuntimeUser, runtime_manager


@override_settings(EMAIL_BACKEND='django.core.mail.backends.console.EmailBackend')
class UserTestCase(TestCase):
    def test_all(self):
        user = RuntimeUser(username='BillieJean',
        ...
        with self.assertRaises(InvalidPassword):
            user.change_password(new_password='you got hacked kiddo',
        ...
        self.assertSetEqual(user.list_items(on_sale=True),
                            {towel, watch})
        ...
        john = RuntimeUser(username='john1144',
        ...
        Transaction.objects.create(source=user.persistent_user,
                                   destination=john.persistent_user,
                                   amount=Decimal(140.00),
                                   item=notepad)
        Transaction.objects.create(source=john.persistent_user,
...
class BiddingTestCase(TestCase):
    def test_increment_bidding_happy_path(self):
        john = RuntimeUser(username='john1144',
                           ...
                           first_name='John',
                           last_name='Sims')
        jimmy = RuntimeUser(username='jimjamjom',
                            email='me@jimjamjom.org',
                            password_raw='12345678',
                            first_name='James',
                            last_name='Hetfield')
        jack = RuntimeUser(username='daniels',
        ...
        scarf = Item.objects.create(title='Scarf',
                                    ...
                                    item_type='Clothing')
        jimmy_has_scarf = UserHasItem.objects.create(item=scarf,
                                                     user=jimmy.persistent_user)

        auction = runtime_manager.create_auction(uhi=jimmy_has_scarf,
